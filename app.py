...
def generate_rag_clinical_assessment(patient_name, prediction_label, confidence, patient_context, language):
    ...
    try:
        ...
        lang_rule = (
            ...
            f"If 'বাংলা', respond entirely in empathetic, respectful, and natural doctor-like formal Bengali."
        )

        system_prompt = (
            f"You are DECat-AI, a helpful and empathetic digital clinician specializing in Diabetes Risk Screening. "
            f"{lang_rule} "
            f"CRITICAL RULES FOR CLINICAL SAFETY:\n"
            f"1. Explain the diagnostic risk dynamically by combining the references and the model's prediction. Speak directly to the patient like a real caring doctor in a real chamber. Be natural, not machine-like.\n"
            f"2. GENTLY BUT CLEARLY ADVISE THE PATIENT TO GET TESTED (HbA1c and Fasting Blood Sugar tests) as a logical next step to be safe, using a friendly medical tone.\n"
            f"3. PROPER INLINE CITATION RULE: You MUST cite your sources inside the text at the immediate end of relevant analytical sentences using formal bracket notation like [1] or [2].\n"
            f"4. Structure cleanly using these translated header fields: 'Diagnostic Guidance', 'Dietary Action Plan', and 'Lifestyle Protocol'.\n"
            f"5. NO SPECIAL SYMBOLS: Never use math characters like >, <, %, $, or markdown asterisks inside paragraph texts. Write them completely as plain textual words (e.g., 'percent', 'greater than', 'শতাংশ')."
        )

        user_payload = (
            f"Patient Name: {patient_name}\n"
            f"CatBoost Model Screening Verdict: {prediction_label} with {confidence} statistical confidence.\n"
            f"Patient Symptoms Data: {patient_context}\n\n"
            f"Retrieved Clinical References:\n{context_str}"
        )
...
def build_clinical_pdf(patient_name, patient_data, verdict, confidence, english_report, citations):
    ...
    title_style = ParagraphStyle('TStyle', parent=styles['Heading1'], fontSize=20, textColor=colors.HexColor('#bd2130'),
                                 alignment=1, spaceAfter=4, fontName='Helvetica-Bold')
    sub_style = ParagraphStyle('SStyle', parent=styles['Normal'], fontSize=9, textColor=colors.HexColor('#555555'),
                               alignment=1, spaceAfter=15, fontName='Helvetica')
    sec_style = ParagraphStyle('SecStyle', parent=styles['Heading2'], fontSize=12, textColor=colors.HexColor('#004085'),
                               spaceBefore=10, spaceAfter=5, fontName='Helvetica-Bold')
    body_style = ParagraphStyle('BStyle', parent=styles['Normal'], fontSize=9.5, leading=14,
                                textColor=colors.HexColor('#222222'), fontName='Helvetica')
    cite_style = ParagraphStyle('CStyle', parent=styles['Normal'], fontSize=8.5, leading=12,
                                textColor=colors.HexColor('#444444'), fontName='Helvetica-Oblique')
    alert_style = ParagraphStyle('AStyle', parent=styles['Normal'], fontSize=8.5, leading=13,
                                 textColor=colors.HexColor('#721c24'), alignment=1, fontName='Helvetica-Bold')
    ...
    story.append(Table([[""]], colWidths=[530], rowHeights=[1.5],
                       style=TableStyle([('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#bd2130'))])))
    ...
    story.append(Table([[""]], colWidths=[530], rowHeights=[0.5],
                       style=TableStyle([('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#cccccc'))])))
    story.append(Spacer(1, 10))
    story.append(Paragraph(
        "Disclaimer: Preliminary computational screening transcript only. This document does not constitute full definitive diagnostics. Kindly coordinate validation diagnostics with a licensed practitioner.",
        alert_style))
...
quiz_schema = [
    ...
    {"field": "Gender", "en": "Select biological sex parameter:", "bn": "আপনার জৈবিক লিঙ্গ নির্বাচন করুন:",
     "options": ["Male", "Female"]},
    {"field": "Polyuria", "en": "Do you experience excessive or unusually frequent urination (Polyuria)?",
     "bn": "আপনার কি অতিরিক্ত বা ঘন ঘন প্রস্রাবের সমস্যা (Polyuria) হচ্ছে?", "options": ["Yes", "No"]},
    {"field": "Polydipsia", "en": "Are you experiencing constant, extreme fluid thirst (Polydipsia)?",
     "bn": "আপনার কি প্রতিনিয়ত অতিরিক্ত বা অস্বাভাবিক তৃষ্ণা (Polydipsia) পাচ্ছে?", "options": ["Yes", "No"]},
    {"field": "Irritability", "en": "Have you noticed any persistent patterns of sudden irritability or mood spikes?",
     "bn": "আপনি কি ইদানীং অতিরিক্ত খিটכיটে মেজাজ বা মানসিক অস্থিরতা অনুভব করছেন?", "options": ["Yes", "No"]},
    {"field": "Itching", "en": "Do you experience localized or generalized recurring skin itching?",
     "bn": "আপনার ত্বকে কি ঘন ঘন বা দীর্ঘস্থায়ী চুলকানির সমস্যা হচ্ছে?", "options": ["Yes", "No"]},
    {"field": "delayed healing",
     "en": "Do surface cuts, scratches, or flesh wounds take a prolonged time to completely heal?",
     "bn": "আপনার শরীরের কোনো ক্ষত, কাটা বা স্ক্র্যাচ শুকাতে কি স্বাভাবিকের চেয়ে বেশি সময় লাগছে?",
     "options": ["Yes", "No"]},
    {"field": "Alopecia", "en": "Are you suffering from active, accelerated hair thinning or loss patches (Alopecia)?",
     "bn": "আপনার কি অতিরিক্ত চুল পড়া বা নির্দিষ্ট স্থান থেকে চুল উঠে যাওয়ার (Alopecia) লক্ষণ দেখা দিচ্ছে?",
     "options": ["Yes", "No"]}
]
...
st.markdown(
    '<span class="header-logo">🩸 DECat‑AI Desk</span><p style="color:#6c757d; font-size:14px; margin-top:-5px;">CatBoost Machine Learning Engine integrated with Real Vector RAG Workspace</p>',
    unsafe_allow_html=True)
st.markdown("---")

for message_bubble in st.session_state.chat_history:
    if message_bubble["role"] == "ai":
        st.markdown(f'<div class="chat-bubble-ai">🤖 <b>DECat-AI:</b> {message_bubble["text"]}</div>',
                    unsafe_allow_html=True)
    else:
        st.markdown(f'<div style="overflow:auto;"><div class="chat-bubble-user">👤 {message_bubble["text"]}</div></div>',
                    unsafe_allow_html=True)
...
if st.session_state.step == -2:
    init_greeting = (
        ...
        f"Before we talk about your health, could you please tell me your full name?"
    ) if lang_selection == "English" else (
        f"হ্যালো! আমি আপনার ডিজিটাল স্বাস্থ্য পরামর্শক হিসেবে আছি। "
        f"আপনার স্বাস্থ্য নিয়ে কথা বলার আগে, আমি কি আপনার সম্পূর্ণ নামটা জানতে পারি?"
    )
    ...
    with st.form(key="identity_node"):
        raw_name = st.text_input("Your Name / আপনার নাম", placeholder="Type here...")
...
# COMPLIANCE SUB-NODE (ইউজারের "No" হ্যান্ডেল করার লজিকসহ বাগ ফিক্সড)
    ...
    consent_prompt = (
        ...
        f"might be related to diabetes. Would you like to check it out with a quick screening test? If you are ready, let's get started!"
    ) if lang_selection == "English" else (
    ...
    with st.form(key=f"survey_form_{st.session_state.step}"):
        if "options" in active_node:
            if active_node["field"] == "Gender":
                ui_labels = ["Male", "Female"] if lang_selection == "English" else ["পুরুষ", "নারী"]
                label_mapper = {"Male": ui_labels[0], "Female": ui_labels[1]}
            else:
            ...
            typed_age = st.number_input("Input biological age:", min_value=1, max_value=122, value=None,
                                        placeholder="Years...")
...
# FINAL METRICS EVALUATION & RAG GENERATION NODE
    ...
    if model is None:
        st.error(
            "❌ Core CatBoost classification configuration binary (.cbm) missing from root directory. Process frozen.")
    else:
        ...
        calculated_confidence = prediction_probabilities[1] * 100 if has_positive_risk else prediction_probabilities[
                                                                                                0] * 100
        ...
        with st.spinner("Invoking active Vector Retrieval & Grounding RAG Synthesis pipeline..."):
            ...
            rag_assessment_report, explicit_citations = generate_rag_clinical_assessment(
                st.session_state.patient_name, verdict_header, formatted_confidence_string, symptoms_query_string,
                lang_selection
            )
        ...
        pdf_binary_stream = build_clinical_pdf(
            st.session_state.patient_name, telemetry_payload, verdict_header, formatted_confidence_string,
            english_pdf_report, explicit_citations
        )
        ...
        st.markdown(
            "<div class='legal-alert'>⚠️ Regulatory Notice: This ecosystem uses computational machine learning classification and real vector metrics retrieval to cross-reference constraints. It does not issue official hospital treatment paths. Please execute proper clinical laboratory tests with a registered physician.</div>",
            unsafe_allow_html=True)
